const FAVICON_BLOCK_START = "<!-- RAAVA_FAVICON_START -->";
const FAVICON_BLOCK_END = "<!-- RAAVA_FAVICON_END -->";
const RUNTIME_BRANDING_BLOCK_START = "<!-- RAAVA_RUNTIME_BRANDING_START -->";
const RUNTIME_BRANDING_BLOCK_END = "<!-- RAAVA_RUNTIME_BRANDING_END -->";
...
export function isWorktreeUiBrandingEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return isTruthyEnvValue(env.RAAVA_IN_WORKTREE);
}
...
export function getWorktreeUiBranding(env: NodeJS.ProcessEnv = process.env): WorktreeUiBranding {
  ...
  const name = nonEmpty(env.RAAVA_WORKTREE_NAME) ?? nonEmpty(env.RAAVA_INSTANCE_ID) ?? "worktree";
  const color = normalizeHexColor(env.RAAVA_WORKTREE_COLOR) ?? deriveColorFromSeed(name);
...
export function renderRuntimeBrandingMeta(branding: WorktreeUiBranding): string {
  ...
  return [
    '<meta name="raava-worktree-enabled" content="true" />',
    `<meta name="raava-worktree-name" content="${escapeHtmlAttribute(branding.name)}" />`,
    `<meta name="raava-worktree-color" content="${escapeHtmlAttribute(branding.color)}" />`,
    `<meta name="raava-worktree-text-color" content="${escapeHtmlAttribute(branding.textColor)}" />`,
  ].join("\n");
}
