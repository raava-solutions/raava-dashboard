...
export function getWorktreeUiBranding(): WorktreeUiBranding | null {
  if (readMetaContent("raava-worktree-enabled") !== "true") return null;

  const name = readMetaContent("raava-worktree-name");
  const color = normalizeHexColor(readMetaContent("raava-worktree-color"));
  ...
  return {
    ...
    name,
    color,
    textColor: normalizeHexColor(readMetaContent("raava-worktree-text-color")) ?? pickReadableTextColor(color),
  };
}
